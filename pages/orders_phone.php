...
for($x=0; $x < $_POST['loop2']; $x++) {
  $stock = htmlspecialchars($_POST['stock'][$x]);
  if($stock == -0) continue;
  $phone_ID = $_POST['id'][$x];
  $price_unit = $_POST['price'][$x];
  $sql = "INSERT INTO phoneitem (ID, weborder_ID, client_ID, phone_ID, price_unit, stock) VALUES (?, ?, ?, ?, ?, ?)";
  $stmt = $connection->prepare($sql);
  $data = array(NULL, $weborder_id, $_SESSION['USER_ID'], $phone_ID, $price_unit, $stock);
